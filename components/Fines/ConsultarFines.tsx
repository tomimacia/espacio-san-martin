...
const ConsultarFines = () => {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setLoadingForm(true);
    await getSingleDoc("Fines", consulta).then((res) => {
      console.log("fetched");
      const registry = res?.data();
      if (!registry) {
        toast({
          title: "No encontrado",
          description: "Regsitrate en Fines o contáctate!",
          status: "warning",
          duration: 9000,
          isClosable: true,
        });
  ...
  return (
    <Flex gap={3} flexDir="column">
      ...
      <Text fontWeight="bold">
        Consulta con tu DNI tus datos o si ya estas registrado
      </Text>
              ...
              <Table variant="striped">
                <Tbody>
                  ...
                  <Tr>
                    <Td>Sede:</Td>
                    <Td>
                      <strong>{registryUser.Sede}</strong>
                    </Td>
                  </Tr>
                  <Tr>
                    <Td>Nacimiento:</Td>
                    <Td>{registryUser.FechaNacimiento}</Td>
                  </Tr>
                  <Tr>
                    <Td>Género:</Td>
                    <Td>{registryUser.Genero}</Td>
                  </Tr>
                  <Tr>
                    <Td>CUIL:</Td>
                    <Td>{registryUser.CUIL}</Td>
                  </Tr>
                </Tbody>
              </Table>
            </TableContainer>
